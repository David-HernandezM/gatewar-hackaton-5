...
class ContractService {
    static async createProgram(sails, signer, initConfig) {
        try {
            const transaction = await sails
                ...
                .withAccount(signer)
                .withValue(BigInt(constants_1.ONE_VARA))
                .calculateGas();
    ...
    static async createPool(factorySails, poolFactorySails, signer, tokenA, tokenB) {
        try {
            ...
            const { response } = await transaction.signAndSend();
            await response();
            const pairAddress = await this.getPairAddress(poolFactorySails, tokenA, tokenB);
    ...
    static async createPoolWithRegisteredToken(factorySails, poolFactorySails, signer, token, registeredToken = null) {
        try {
            const transaction = await factorySails
                .services
                .Service
    ...
    static async getPairAddress(poolFactorySails, tokenA, tokenB) {
        try {
    ...
    static async createProgramAndPool(factorySails, poolFactorySails, signer, initConfig, registeredToken = null) {
        try {
            const programResponse = await this.createProgram(factorySails, signer, initConfig);
            let tokenAddress;
            if (programResponse.programCreated) {
                tokenAddress = programResponse.programCreated.address;
            }
            else {
            ...
            const pairAddress = await this.createPoolWithRegisteredToken(factorySails, poolFactorySails, signer, tokenAddress, registeredToken);
            return {
                programResponse,
                pairAddress
            };
    ...
    static async getIdToAddress(sails) {
        try {
            const response = await sails
                .services
                .Service
    ...
    static async getRegistry(sails) {
        try {
            const response = await sails
                .services
                .Service
    ...
    static async getPoolFactoryAddress(sails) {
        try {
            const response = await sails
                .services
                .Service
    ...
    static async getAllPairs(poolFactorySails) {
        try {
            const response = await poolFactorySails
                .services
                .Factory
    ...
    static async getFeeTo(poolFactorySails) {
        try {
            const response = await poolFactorySails
                .services
                .Factory
    ...
    static async getTreasuryId(poolFactorySails) {
        try {
            const response = await poolFactorySails
                .services
                .Factory
